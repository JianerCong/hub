...
#+begin_src c++
  ...
  BOOST_AUTO_TEST_CASE(test_opendb) {
    ...
    rocksdb::Status status = rocksdb::DB::Open(options, string(d) , &db);
  ...
  struct D{
    D(){
      ...
      // rocksdb::Status status = rocksdb::DB::Open(options,"/tmp/testdb", &db);
      rocksdb::Status status = rocksdb::DB::Open(options, string(dbDir) , &db);
      BOOST_REQUIRE(status.ok());
  }
    ~D(){
  ...
  BOOST_AUTO_TEST_CASE(manually_backup){
    ...
    // Create db
    rocksdb::DB* db;
    rocksdb::Status status = rocksdb::DB::Open(options, string(d1) , &db);
    BOOST_REQUIRE(status.ok());
    ...
    // open the db in d2
    string value;
    status = rocksdb::DB::Open(options, string(d2) , &db);
    BOOST_REQUIRE(status.ok());
  ...
  // BOOST_AUTO_TEST_CASE(test_batch_write){
  //   rocksdb::DB* db = initDb();
  //   rocksdb::Status status = rocksdb::DB::Open(options, "/tmp/testdb", &db);
  //   BOOST_REQUIRE(status.ok());
