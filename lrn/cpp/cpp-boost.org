...
#include <functional>
using std::function;
void time_it(function<void(void)> f, double min, double max){
  time_point<system_clock> start, end;
  start = system_clock::now();
  BOOST_TEST_MESSAGE("Before timing");
  ...
  BOOST_TEST_MESSAGE((format("Duration %d secs") % s).str()
                     );
  BOOST_CHECK(s > min && s < max);  // slept for 3sec
}

BOOST_AUTO_TEST_CASE(test_basic_sleep){
  boost::asio::io_context io;

  /*
    🐢 : The important thing about asio is the `async_sleep` which will give
    controls to other coroutine.

    🦜 :
   */
  time_it([&](){
    boost::asio::steady_timer(io, boost::asio::chrono::seconds(3)).wait();
  }, 2, 4);
}

/*
  🦜 : Okay, well then, how do we 'run' two tasks in parallel?

  🐢 : You use post() or dispatch(). Which is kinda like saying "hey, execute
  this". The difference is that post(f) must be wait for the current handler to
  finish, while the dispatch will just start the job.

  🦜 : So which should I use.

  🐢 : Let's try post() because it's shorter
 */
using boost::asio::io_context;
void asio_sleep(int x=2){
  boost::asio::io_context io;
  boost::asio::steady_timer(io, boost::asio::chrono::seconds(x)).wait();
}

BOOST_AUTO_TEST_CASE(test_post){
  auto f1 = [](){
    BOOST_TEST_MESSAGE("111");
    asio_sleep(3);
    BOOST_TEST_MESSAGE("111");
  };

  auto f2 = [](){
    BOOST_TEST_MESSAGE("222");
    asio_sleep(3);
    BOOST_TEST_MESSAGE("222");
  };
  // boost::asio::io_context io_context;
  boost::asio::dispatch(f1);
  boost::asio::dispatch(f2);
  BOOST_TEST_MESSAGE("Finished");

  /* 🦜 : They run in series, it looks like.*/
  }
...
#+begin_src powershell
  ...
  # 🦜 : It looks like both the above two won't work
  .\b2.exe
