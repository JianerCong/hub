* Emac
** start as a server:
🦜 : I defined a helper function called (start-my-emacs-server), which will
start listening on port 1234

Now to invoke it, do :
#+begin_src bash
  server_file= ~/.emacs.d/server/server
  emacsclient hi.txt -f $server_file -c
  # -c,--create-frame : create a new frame
  # -f : use the server file, which contains info like the port number
#+end_src
...
(
 (nil . ((indent-tabs-mode . t)
         (fill-column . 80)
         (mode . auto-fill))            ;special element `mode': enable auto-fill mode
      )

 ;; major mode name ⇒ alist
 (c-mode . (
            (c-file-style . "BSD")
            (subdirs . nil)             ;special keyword `subdirs' : settings not applied
                                        ;recursively.
            )
         )
...
*** set dir local-var without creating a file
#+begin_src lisp
