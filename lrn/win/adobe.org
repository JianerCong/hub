...
+ C-S-' :: snap to grid
*** shape tool
 + To select a vertex, click the vertex with the Selection tool. To add
   vertices, Shift-click them.
**** how to stop a path
1. go back to the first vertex
2. double-clicking the last vertex
3. choosing Layer > Mask and Shape Path > Closed
4. To leave the path open, activate a different tool
5. [F2] tp deselect the path
