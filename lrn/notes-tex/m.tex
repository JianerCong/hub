...
\begin{tikzpicture}
  ...
  \matrix (M1) [matrix of nodes,row sep=1.5cm,matrix anchor=south]{
    \dns\\
    \dns\\
    \dns\\
    \dns\\
  ...
  \newcommand{\comeAndGo}[4]{
    \draw[-latex] (#1) to[out = 180, in=180] node[auto,swap] {#3} (#2);
    \draw[-latex] (#2) to[out = 0, in=0] node[auto,swap] {#4} (#1);
  }
