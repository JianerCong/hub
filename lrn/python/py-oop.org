...
#+end_src
** the descripter (attach properties to a class)
#+begin_src python
class Ten:
    def __get__(self, obj, objtype=None):
        return 10
class A:
    x = 5                       # regular class attr
    y = Ten()                   # a Descriptor

def test_one():
    a = A()
    assert a.y == 10            # look up the descriptor and call __get__
    assert a.x == 5             # look up the obj class dict


class XPlusOne:
    def __get__(self, obj, objtype=None):
        # objtype is the attached type
        return obj.x + 1
...
## 🦜 : Each descriptor can use a custom accessor
import logging
logging.basicConfig(level=logging.INFO)
...
class Person:
    ...
    def birthday(self):
        self.age += 1
def test_vars():
    assert vars(vars(Person)['name']) == {'public_name': 'name',
                                          'private_name' : '_name'}

    assert vars(vars(Person)['age']) == {'public_name': 'age',
                                          'private_name' : '_age'}

#+end_src
** bind class method: (natural)
#+begin_src python
o = []
